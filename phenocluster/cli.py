...
def validate_config(
        ...
        if cfg.multistate.transitions:
            for tr in cfg.multistate.transitions:
                ...
                if tr.to_state not in state_ids:
                    errors.append(
                        f"Transition '{tr.name}' references unknown to_state {tr.to_state}"
                    )

    # Reference phenotype
    if cfg.reference_phenotype.strategy not in ("largest", "healthiest", "specific"):
        errors.append(
            f"reference_phenotype.strategy must be 'largest', 'healthiest', or 'specific', "
            f"got '{cfg.reference_phenotype.strategy}'"
        )
    ...
    # Cross-reference against data CSV
    csv_columns = None
    if data is not None:
        try:
        ...
        def _check_columns(col_list, section_name):
        ...
        # Feature columns
        _check_columns(cfg.continuous_columns, "data.continuous_columns")
        _check_columns(cfg.categorical_columns, "data.categorical_columns")
        if cfg.outcome.enabled:
            _check_columns(cfg.outcome.outcome_columns, "outcome.outcome_columns")

        # Stratify-by column
        if cfg.data_split.stratify_by:
            _check_columns([cfg.data_split.stratify_by], "data.split.stratify_by")
        ...
        # Survival time/event columns
        if cfg.survival.enabled and cfg.survival.targets:
            for t in cfg.survival.targets:
                _check_columns([t.time_column], f"survival.targets[{t.name}].time_column")
                _check_columns([t.event_column], f"survival.targets[{t.name}].event_column")

        # Multistate event/time columns
        if cfg.multistate.enabled and cfg.multistate.states:
            for s in cfg.multistate.states:
                if s.event_column:
                    _check_columns([s.event_column], f"multistate.states[{s.name}].event_column")
                if s.time_column:
                    _check_columns([s.time_column], f"multistate.states[{s.name}].time_column")
            if cfg.multistate.baseline_confounders:
                _check_columns(
                    cfg.multistate.baseline_confounders, "multistate.baseline_confounders"
                )

        # Reference phenotype health outcome
        if (
            cfg.reference_phenotype.strategy == "healthiest"
            and cfg.reference_phenotype.health_outcome
        ):
            _check_columns(
        ...
        # Warn about duplicate columns across feature lists
        overlap = set(cfg.continuous_columns) & set(cfg.categorical_columns)
        if overlap:
            warnings.append(f"Columns in both continuous and categorical: {sorted(overlap)}")
